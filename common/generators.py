...
class ChunkedGenerator_Seq:
    ...
    """
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, action,
        ...
        self.batch_act = np.empty((batch_size, 1), dtype=object)
    ...
    def next_pairs(self):
        if self.state is None:
            if self.shuffle:
                # randomly shuffle the pairs per epoch
                pairs = self.random.permutation(self.pairs)
            else:
                # keep the pairs in the original order
                pairs = self.pairs
            return 0, pairs
        else:
            return self.state
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    self.batch_act[i] = self.action[seq_i]

                if self.endless:
                    self.state = (b_i + 1, pairs)
                if self.poses_3d is None and self.cameras is None:
                    yield None, None, self.batch_2d[:len(chunks)], self.batch_act[:len(chunks)]
                elif self.poses_3d is not None and self.cameras is None:
                    yield None, self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_act[:len(chunks)]
                elif self.poses_3d is None:
                    yield self.batch_cam[:len(chunks)], None, self.batch_2d[:len(chunks)], self.batch_act[:len(chunks)]
                else:
                    yield self.batch_cam[:len(chunks)], self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_act[:len(chunks)]
